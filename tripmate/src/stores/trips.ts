...
import type { Trip, Expense, ChatMessage, TripEvent, Task, Poll, Link, Fund } from '../types'
import {
...
export const useTripsStore = defineStore('trips', () => {
  ...
  function calculateBalances(tripId: string) {
    const shared = getSharedExpenses(tripId)
  ...
  function toggleTask(taskId: string, userId: string) {
    ...
    if (task) {
      ...
      task.completedBy = task.isCompleted ? userId : undefined
    }
  }
  ...
  function votePoll(pollId: string, optionIndex: number, userId: string) {
    ...
    poll.options.forEach((opt, i) => {
      ...
      if (i === optionIndex) opt.votes.push(userId)
    })
  }
