  ...
  <ion-page>
        ...
        <ion-list>
          <ion-item-sliding v-for="task in getTasksBySection(section)" :key="task.id">
            ...
            <ion-item-options side="end">
              <ion-item-option color="primary" @click="openEditTask(task)">✏️</ion-item-option>
              <ion-item-option color="danger" @click="confirmDeleteTask(task)">🗑</ion-item-option>
            </ion-item-options>
          </ion-item-sliding>
        </ion-list>
      </template>
    ...
    <ion-modal :is-open="showModal" @did-dismiss="showModal = false">
      ...
      <ion-content class="ion-padding">
        <ion-item>
          <ion-input v-model="formTitle" label="Задача" label-placement="floating" placeholder="Что нужно сделать" />
        </ion-item>
        <ion-item>
...
const allSectionOptions = computed(() => {
...
const deleteAlertButtons = computed(() => [
  ...
  { text: 'Удалить', role: 'destructive', handler: () => { store.deleteTask(deletingId.value) } },
])
...
function openAddTask() {
  ...
  formTitle.value = ''
  formSection.value = allSectionOptions.value[0] || 'До поездки'
  formAssignee.value = ''
...
function openEditTask(task: Task) {
  ...
  formSection.value = task.section
  formAssignee.value = task.assigneeId ?? ''
  newSectionName.value = ''
  showModal.value = true
}
...
function saveTask() {
  ...
  if (editingTaskId.value) {
    store.updateTask(editingTaskId.value, {
      ...
      section,
      assigneeId: formAssignee.value || undefined,
    })
  } else {
    store.addTask(tripId.value, formTitle.value, section, formAssignee.value || undefined, auth.user?.id ?? '')
  }
...
function confirmDeleteTask(task: Task) {
  ...
  deletingTitle.value = task.title
